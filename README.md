...
# GPLaSDI with neural operator autoencoder

It should work with the same environment that regular GPLaSDI was, but if that fails, use the environment.yml to create a conda environment with those dependencies. New files that differ from the original GPLaSDI are that fno_shared.py contains the helper functions for the fno. FNO_Autoencoder_1d is now an option for "latent space" in workflow.py. 2 and higher dimensions are unimplemented currently. FNO_Autoencoder_1d is a class in latent_space.py. The currently implemented version is a severely shortened version of the FNO usually used in order to try to make it faster. examples/FAE-experiments contains the experiments on burgerd1d with FNO autoencoder. The .ipynb compares the current results. The issue is that even though the FAE contains way fewer model parameters, it takes too long to evaluate so has bad comparitive error. 
