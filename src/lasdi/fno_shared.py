...
def get_grid1d(shape, device):
    ...
    gridx = gridx.reshape(1, 1, size_x).repeat([shape[0],1 , 1])
...
class LinearDecoder1d(nn.Module):
    ...
    def forward(self, x, s,act = False):
        """
...
class NonlinearDecoder1d(nn.Module):
    def __init__(self, in_channels, out_channels, modes1, act):
        """
        Fourier neural decoder layer for functions over the torus. Maps vectors to functions.
        Inputs:    
            in_channels  (int): dimension of input vectors
            out_channels (int): total number of functions to extract
        """
        super(NonlinearDecoder1d, self).__init__()
...
class SubsampleScheduler:
    ...
    '''
    def __init__(self, ss_max, ndim=2, threshold=0.9, patience=10, max_iter = 1000):
        self.ss_max = ss_max # maximum subsampling
        self.ss = ss_max # current subsampling rate
        self.num_rates = ss_max.bit_length() # number of subsampling rates
        self.ndim = ndim
    ...
    def step(self,loss):
        ...
        if self.num_bad_epochs > self.patience or self.last_epoch >= self.max_iter//self.num_rates:
